...
export async function getAgencies() {
  return sql`SELECT id, name, url, logo_small, locality FROM agencies ORDER BY name`;
}
...
export async function getAgenciesPage({ page = 1, perPage = 5 }) {
  ...
  const rows = await sql`
    SELECT id, name, url, logo_small, locality, count(*) OVER()::int AS total
    FROM agencies
    ORDER BY name
...
export async function getAgency(id) {
  const rows = await sql`SELECT id, name, url, logo_small, ico, locality FROM agencies WHERE id = ${id}`;
  return rows[0] ?? null;
}

// Deterministic, valid 8-digit Czech IČO derived from a company id
// (7-digit base + mod-11 check digit). Matches scripts/seed.mjs.
export function icoFromId(id) {
  const n = (Math.abs(Number(id)) * 2654435761) >>> 0;
  const base = String(1000000 + (n % 9000000));
...
// Single-company detail: includes IČO (which the list endpoint omits).
export async function getAgencyDetail(id) {
  const rows = await sql`SELECT id, name, url, ico, locality FROM agencies WHERE id = ${id}`;
  const a = rows[0];
  if (!a) return null;
  const cnt = await sql`SELECT count(*)::int AS n FROM listings WHERE company_id = ${id}`;
  return {
    id: a.id,
    name: a.name,
    locality: a.locality ?? await derivedLocality(id),
    url: a.url,
    ico: a.ico ?? icoFromId(a.id),
    num_adverts: cnt[0].n,
  };
}

// Most-common city across an agency's listings (fallback when no manual override).
async function derivedLocality(id) {
  const locs = await sql`SELECT locality FROM listings WHERE company_id = ${id} AND locality IS NOT NULL`;
  const cityCounts = {};
  for (const { locality } of locs) {
    const c = cityFromLocality(locality);
    if (c) cityCounts[c] = (cityCounts[c] ?? 0) + 1;
  }
  return Object.entries(cityCounts).sort((x, y) => y[1] - x[1])[0]?.[0] ?? null;
}

const norm = (v) => (v != null && String(v).trim() ? String(v).trim() : null);

export async function createAgency({ id, name, url = null, logo_small = null, ico = null, locality = null }) {
  const rows = await sql`
    INSERT INTO agencies (id, name, url, logo_small, ico, locality)
    VALUES (${id}, ${name}, ${url}, ${logo_small}, ${ico ?? icoFromId(id)}, ${norm(locality)})
    RETURNING id, name, url, logo_small, ico, locality`;
  return rows[0];
}
