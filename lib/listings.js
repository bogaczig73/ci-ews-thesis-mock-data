...
export async function getAgency(id) {
  const rows = await sql`SELECT id, name, url, logo_small, ico FROM agencies WHERE id = ${id}`;
  return rows[0] ?? null;
}
...
export async function getAgencyDetail(id) {
  const rows = await sql`SELECT id, name, url, ico FROM agencies WHERE id = ${id}`;
  const a = rows[0];
  if (!a) return null;
  ...
  const locality = Object.entries(cityCounts).sort((x, y) => y[1] - x[1])[0]?.[0] ?? null;
  return {
    id: a.id,
    name: a.name,
    locality,
...
export async function createAgency({ id, name, url = null, logo_small = null, ico = null }) {
  const rows = await sql`
    INSERT INTO agencies (id, name, url, logo_small, ico)
    VALUES (${id}, ${name}, ${url}, ${logo_small}, ${ico ?? icoFromId(id)})
    RETURNING id, name, url, logo_small, ico`;
  return rows[0];
}
